    ...
    public Boolean saveMoney(String card,String pwd,String moneyNum){
        ...
        synchronized (ATM.class) {
            if (u.getU_login_name().equals(card) && u.getU_login_pwd().equals(pwd)) {
                ...
                System.out.println(Thread.currentThread().getName()+"存钱---->"+u.getU_name()+"在"+
                        new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(u.getSave_money_time())+
                        "存["+moneyNum+"]元，余额："+u.getU_wallet());
                return true;
            }
        }
        ...
        synchronized (ATM.class) {
            if (u!=null && u.getU_login_name().equals(card) && u.getU_login_pwd().equals(pwd)) {
                ...
                if(oldData.compareTo(money)>=0){
                    ...
                    System.out.println(Thread.currentThread().getName()+"取钱---->"+u.getU_name()+"在"+
                            new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(u.getDraw_money_time())+
                            "取["+moneyNum+"]元,余额："+u.getU_wallet());
                    return true;
                }else {
